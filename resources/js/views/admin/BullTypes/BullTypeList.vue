<template>
    <div class="container-fluid">
        <breadcrumb :options="['Bull Type List']">
            <button class="btn btn-primary"  @click="addListDataModal()">Add Bull Type</button>
        </breadcrumb>
        ...
        <add-edit-bullTypeListData @changeStatus="changeStatus" v-if="loading"/>
...
export default {
    ...
    data() {
        return {
            tableOptions: {
                source: 'admin/breeding/bullTypeList',
                search: true,
                slots: [3],
    ...
    methods: {
        ...
        addListDataModal(row = '') {
            this.loading = true;
            setTimeout(() => {
                bus.$emit('add-edit-bullTypeListData', row);
            })
        },
        changePassword(row) {
            this.loading = true;
